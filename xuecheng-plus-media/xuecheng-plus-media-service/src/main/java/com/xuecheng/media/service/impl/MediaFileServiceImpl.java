...
public class MediaFileServiceImpl implements MediaFileService {
 ...
 public UploadFileResultDto uploadFile(Long companyId, UploadFileParamsDto uploadFileParamsDto, String localFilePath, String objectName) {
  ...
  //先得到拓展名
  //filename.lastIndexOf(".")：找到字符串 filename 中「最后一个小数点 .」的索引位置（字符串索引从 0 开始）
  //filename.substring(index)：从指定的索引位置开始，截取字符串的后半部分（包括该索引对应的字符），直到字符串末尾。
  ...
  //将文件信息保存到数据库 - media_files
  //bucket_mediafiles：桶的类型 - 存储普通文件
...
/*检查分块是否已经存在*/
 ...
 private void addWaitingTask(MediaFiles mediaFiles){
  ...
  String extension = filename.substring(filename.lastIndexOf("."));
  String mimeType = getMimeType(extension);
  if(mimeType.equals("video/x-msvideo")){  //通过mimeType判断如果是avi视频则写入待处理任务
