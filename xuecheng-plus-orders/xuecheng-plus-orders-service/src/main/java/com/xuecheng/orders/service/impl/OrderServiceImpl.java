...
public class OrderServiceImpl implements OrderService {
    ...
    public PayStatusDto queryPayResultFromAlipay(String payNo){
        ...
        String body = null;
        try {
            AlipayTradeQueryResponse response = alipayClient.execute(request);  //通过alipayClient调用API，获取对应的response类
        ...
        // 1. 解析支付宝返回的完整JSON（用JSONObject，不用Map）
        JSONObject bodyJson = JSONObject.parseObject(body);
        // 2. 安全获取内层响应对象（核心修复！）
        JSONObject responseJson = bodyJson.getJSONObject("alipay_trade_query_response");

        //解析支付结果
        PayStatusDto payStatusDto = new PayStatusDto();
        payStatusDto.setOut_trade_no(payNo);
        payStatusDto.setTrade_no(responseJson.getString("trade_no"));   //支付宝的交易号
        payStatusDto.setTrade_status(responseJson.getString("trade_status"));
        payStatusDto.setApp_id(APP_ID);
        payStatusDto.setTotal_amount(responseJson.getString("total_amount"));
    ...
    public XcOrders saveXcOrders(String userId, AddOrderDto addOrderDto){
        ...
        //订单明细表
        //将前端传入的明细json串转成List
        String orederDetailJson = addOrderDto.getOrderDetail();
        List<XcOrdersGoods> xcOrdersGoods = JSON.parseArray(orederDetailJson, XcOrdersGoods.class);
